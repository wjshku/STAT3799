...
```{r}
esti_error <- function(n,h,train_label){
  esti <- train_label$Y
...
nw_1_cv <- function(hlist = seq(0.1,1.4,0.1),train_label){
  ...
  for(h in hlist){
    ...
    for(exp in 2:nrow(train_label)){
        ...
        test_x <- (train_label[exp-1,1]+train_label[exp,1])
...
hlist <- seq(0.1,0.3,0.1)
glist <- seq(0.3,0.3,0.3)
...
Test the true coverage probability of proposed confidence interval
```{r}
set.seed(60)
