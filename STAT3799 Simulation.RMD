...
data <- simu_norm(2, para$n, para$m, meanx = 0, sigmax = sigmax, sigmaep = sigmaep)
```
...
plot(mMSE_HY~h,data = filter(result_hy, g == result_hy$g[HY_opt]),main = "g = 3.25")
...
para <- paraset_g(128,15/19,rk$value, muk$value)
```
...
for(i in 1:rounds){
  ...
  coverage <- rbind(coverage,CI_1(adjust = 3,0.95,para,train_label,train_un,test,sigmaep,rk$value,sigmak$value))
}
...
for(i in 1:length(hlist)){
  ...
  for(j in 1:length(glist)){
    ...
    coverage <- rbind(coverage,data.frame(h = para$h, g = para$g, cover = mean(cover, na.rm = T)))
  }
}
```
